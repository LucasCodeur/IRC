	...
	for (size_t i = 0; i < params.size(); i++)
	{
		...
		if (it != chanMap.end())
		{
			if (it->second->removeUser(fd))
				...
				std::cout << DBUG << fd << RED " could NOT leave : " << it->first << " not a member" << RESET << std::endl;
		}
		else
		{
			std::cout << DBUG << fd << RED " could NOT leave : " << params[i] << " channel not found" << RESET << std::endl;
		}
