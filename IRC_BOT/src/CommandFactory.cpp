...
Command *CommandFactory::createCommand(Server *server, const int clientFd, const std::string str)
{
	...
	const char		*types[14] = {"", "JOIN", "PRIVMSG", "KICK", "INVITE", "TOPIC", "MODE", "WHO", "PASS", "NICK", "USER", "PART", "QUIT", "WELCOME"};
	...
	creators[13] = &CommandFactory::createWelcomeCommand;
...
Command *CommandFactory::createWelcomeCommand(Server *server, const int clientFd, Command::t_msgSpecs specs, const std::vector<std::vector<std::string> > params)
{
	return (new WelcomeCommand(server, clientFd, specs, params));
}
